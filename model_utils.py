...
class ModelMaker:
    ...
    def _make_pipeline(self):
        ...
        if self.categorical_vars:
            pipeline_categorical = Pipeline([('categorical_selector', ColumnSelector(self.categorical_vars)),
                                             ('categorical_imputer', SimpleImputer(strategy='most_frequent')),
                                             ('one_hot_encoder', OneHotEncoder(sparse=False,
                                                                               handle_unknown='ignore'))])

        if bool(self.numerical_vars) & bool(self.scale_vars):
            pipeline_numeric = Pipeline([('numeric_selector', ColumnSelector(self.numerical_vars)),
                                         ('scaling', StandardScaler()),
                                         ('numeric_imputer', SimpleImputer(strategy=self.imputation_numeric))])
        elif self.numerical_vars:
            pipeline_numeric = Pipeline([('numeric_selector', ColumnSelector(self.numerical_vars)),
                                         ('numeric_imputer', SimpleImputer(strategy=self.imputation_numeric))])

        if bool(self.categorical_vars) & bool(self.numerical_vars):
            pipeline_preprocessing = FeatureUnion([('categorical_pipeline', pipeline_categorical),
                                                   ('numeric_pipeline', pipeline_numeric)])
        else:
            pipeline_preprocessing = (pipeline_numeric or pipeline_categorical)

        self.pipeline_model = Pipeline([('preprocessing', pipeline_preprocessing),
    ...
    def get_coeffitients(self):
        ...
        return pd.DataFrame({'var_name': self.variable_names,
                             'coef': coeff}).sort_values('coef',
                                                         ascending=False)
    ...
    def visualize_precision_recall(self, X_test_, y_test_):
        if self.pipeline_model:
            ...
            precision, recall, thresholds = precision_recall_curve(y_test_,
                                                                   self.pipeline_model.predict_proba(X_test_)
                                                                   [:, 1])
    ...
    def get_feature_importance(self, plot=True):
        ...
        out = (pd.DataFrame({'feature': self.variable_names,
                             'importance': importances})
                             .sort_values('importance', ascending=False))

        fig, ax = plt.subplots(figsize=(15, 8))
        out.set_index('feature').plot(kind='bar', ax=ax)
        return out
